...
def _get_fixup_kernel(
    ...
    if kernel_kind not in ("simt_row4", "simt_row8"):
        raise ValueError(
            "the SM80 CP fixup builds simt_row4 and simt_row8, "
            f"got {kernel_kind}"
        )
...
def cp_delta_rule_fixup_dsl_sm80(
    ...
    *,
    ...
    compiled = get_cached_compile(kernel, _sm80_cp_compile_options(device))
    if compiled is None:
        ...
        compiled = cached_compile(
            kernel, *kernel_args, compile_options=_sm80_cp_compile_options(device)
        )
    if _plan_sink is not None:
        _plan_sink.append((compiled, (
            local_transfer_tma,
...
#
# Ported in pieces. `run_aux_loop_body`, `run_aux_math_role` and
# `run_cp_state_math_role` carry no architecture-specific code at all and are
# taken as they stand. `load_t_tma`, `run_load_t_role` and `cp_qk_and_t_epi`
...
class CPDeltaRulePrefillSm80(_FullyFusedDeltaRuleSm80):
    ...
    def kk_epi(
        ...
        The GEMM that filled the accumulator still runs. It is dead work and
        removing it is a separate change from making this correct.
        """
    ...
    # The sm90 CP version of this takes o, qk and kk pipelines, because sm90
    # splits the work across roles that hand tiles to each other. sm80
    # collapsed those roles, so this is the fused sm80 math role with the CP
    # delta applied instead -- same recurrence, chunk-shaped bookkeeping.
    @cute.jit
